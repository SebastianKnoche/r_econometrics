...
7. Run a simple regression where price is only regressed on `bdrms` and compare the `bdrms`-coefficient from this regression with $\hat{\beta_{2}}$ (from part 2). Now confirm the partialling out
interpretation of OLS estimates by following the partialling out procedure. This first
requires regressing `bdrms` on `sqrft` and saving the residuals, $\hat{r_{1}}$. Then regress price on $\hat{r_{1}}$ and compare the coefficient on $\hat{r_{1}}$ with $\hat{\beta_{2}}$.
...
print(paste("           = 0 +", my_lm_wage_1, "* 15 + 0 =", my_lm_wage_1 * 15))
...
```{r}
print(paste("log(wage) =", my_lm_wage_0, "+", my_lm_wage_1, "IQ + u"))
# print("log(delta-wage) = delta-β0 + delta-β1 IQ + delta-u")
# print(paste("                = 0 +", my_lm_wage_1, "* 15 + 0 =", my_lm_wage_1 * 15))
# log(my_lm_wage_1 * 15)
# log(my_lm_wage_1)+log(15)
# my_lm_wage_1+15
```
